...
template<>
__global__ void fft_radix_2<float2, 15, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (32768 * BS + 1024 - 1) / 1024; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    global_k += tx / 8;
        ...
        delta_angle.x = __cosf(global_j *  -0.003067961661145091f);
        delta_angle.y = __sinf(global_j *  -0.003067961661145091f);
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 15, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    global_k += tx / 8;
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(16 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(16 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(16 + tx / 8) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[2],r[(32 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(32 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(32 + tx / 8) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(80 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(80 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(80 + tx / 8) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(112 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(112 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(112 + tx / 8) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 8 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 128: 0;
        // tmp.y = (tx / 8 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 128: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 15, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
