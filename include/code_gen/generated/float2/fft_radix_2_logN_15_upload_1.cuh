...
template<>
__global__ void fft_radix_2<float2, 15, 1, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 15, 1, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    gPtr += (bx % BS * 32768);
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(32 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(32 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(32 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(96 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(96 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(96 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[3],r[(160 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(160 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(160 + tx / 4) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[6].y + rPtr[6].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[6].y + rPtr[6].x);
        turboFFT_ZMUL(tmp, rPtr[6],r[(192 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[6], r[(192 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[6], r[(192 + tx / 4) % 3])
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 15, 1, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
