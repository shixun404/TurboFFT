...
template<>
__global__ void fft_radix_2<float2, 5, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (32 * BS + 64 - 1) / 64; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    rPtr[1] = shPtr[offset + 8];
    
    rPtr[2] = shPtr[offset + 16];
    
    rPtr[3] = shPtr[offset + 24];
    
    rPtr[4] = shPtr[offset + 32];
    
    rPtr[5] = shPtr[offset + 40];
    
    rPtr[6] = shPtr[offset + 48];
    
    rPtr[7] = shPtr[offset + 56];
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 5, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[0] = *(checksum_DFT + 32 - 2 + tx + 0);
    shPtr[tx + 0] = rPtr_2[0];
    
    rPtr_2[1] = *(checksum_DFT + 32 - 2 + tx + 8);
    shPtr[tx + 8] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 32 - 2 + tx + 16);
    shPtr[tx + 16] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 32 - 2 + tx + 24);
    ...
    gPtr += (bx % BS * 64);
        ...
        // tmp_3.x += bid_cnt * (rPtr[0].x + rPtr[0].y) * 32;
    ...
    gPtr += (bx % BS * 64);
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(4 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(4 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(4 + tx / 2) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(12 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(12 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(12 + tx / 2) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[3],r[(20 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(20 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(20 + tx / 2) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(28 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(28 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(28 + tx / 2) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 2 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 32: 0;
        // tmp.y = (tx / 2 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 32: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 5, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
