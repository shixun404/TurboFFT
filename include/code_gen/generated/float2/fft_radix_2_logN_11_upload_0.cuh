...
template<>
__global__ void fft_radix_2<float2, 11, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (2048 * BS + 2048 - 1) / 2048; bid += delta_bid)
    {
    bid_cnt += 1;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 11, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[1] = *(checksum_DFT + 2048 - 2 + tx + 128);
    shPtr[tx + 128] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 2048 - 2 + tx + 256);
    shPtr[tx + 256] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 2048 - 2 + tx + 384);
    shPtr[tx + 384] = rPtr_2[3];
    
    rPtr_2[4] = *(checksum_DFT + 2048 - 2 + tx + 512);
    shPtr[tx + 512] = rPtr_2[4];
    
    rPtr_2[5] = *(checksum_DFT + 2048 - 2 + tx + 640);
    shPtr[tx + 640] = rPtr_2[5];
    
    rPtr_2[6] = *(checksum_DFT + 2048 - 2 + tx + 768);
    shPtr[tx + 768] = rPtr_2[6];
    
    rPtr_2[7] = *(checksum_DFT + 2048 - 2 + tx + 896);
    shPtr[tx + 896] = rPtr_2[7];
    
    rPtr_2[8] = *(checksum_DFT + 2048 - 2 + tx + 1024);
    shPtr[tx + 1024] = rPtr_2[8];
    
    rPtr_2[9] = *(checksum_DFT + 2048 - 2 + tx + 1152);
    shPtr[tx + 1152] = rPtr_2[9];
    
    rPtr_2[10] = *(checksum_DFT + 2048 - 2 + tx + 1280);
    shPtr[tx + 1280] = rPtr_2[10];
    
    rPtr_2[11] = *(checksum_DFT + 2048 - 2 + tx + 1408);
    shPtr[tx + 1408] = rPtr_2[11];
    
    rPtr_2[12] = *(checksum_DFT + 2048 - 2 + tx + 1536);
    shPtr[tx + 1536] = rPtr_2[12];
    
    rPtr_2[13] = *(checksum_DFT + 2048 - 2 + tx + 1664);
    shPtr[tx + 1664] = rPtr_2[13];
    
    rPtr_2[14] = *(checksum_DFT + 2048 - 2 + tx + 1792);
    shPtr[tx + 1792] = rPtr_2[14];
    
    rPtr_2[15] = *(checksum_DFT + 2048 - 2 + tx + 1920);
    shPtr[tx + 1920] = rPtr_2[15];
    ...
    gPtr += (bx % BS * 2048);
        ...
        turboFFT_ZMUL(tmp, rPtr[9],r[(384 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[9], r[(384 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[9], r[(384 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[13],r[(896 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[13], r[(896 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[13], r[(896 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[2].y + rPtr[2].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[2].y + rPtr[2].x);
        turboFFT_ZMUL(tmp, rPtr[2],r[(1024 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(1024 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(1024 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[3].y + rPtr[3].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[3].y + rPtr[3].x);
        turboFFT_ZMUL(tmp, rPtr[3],r[(1152 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(1152 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(1152 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[10].y + rPtr[10].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[10].y + rPtr[10].x);
        turboFFT_ZMUL(tmp, rPtr[10],r[(1280 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[10], r[(1280 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[10], r[(1280 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[11].y + rPtr[11].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[11].y + rPtr[11].x);
        turboFFT_ZMUL(tmp, rPtr[11],r[(1408 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[11], r[(1408 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[11], r[(1408 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[6].y + rPtr[6].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[6].y + rPtr[6].x);
        turboFFT_ZMUL(tmp, rPtr[6],r[(1536 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[6], r[(1536 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[6], r[(1536 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[7].y + rPtr[7].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[7].y + rPtr[7].x);
        turboFFT_ZMUL(tmp, rPtr[7],r[(1664 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(1664 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(1664 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[14].y + rPtr[14].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[14].y + rPtr[14].x);
        turboFFT_ZMUL(tmp, rPtr[14],r[(1792 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[14], r[(1792 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[14], r[(1792 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[15].y + rPtr[15].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[15].y + rPtr[15].x);
        turboFFT_ZMUL(tmp, rPtr[15],r[(1920 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[15], r[(1920 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[15], r[(1920 + tx / 1) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 1 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 2048: 0;
        // tmp.y = (tx / 1 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 2048: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 11, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
