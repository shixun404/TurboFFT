...
template<>
__global__ void fft_radix_2<float2, 12, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (4096 * BS + 4096 - 1) / 4096; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    delta_angle.x = __cosf(j * -0.0015339808305725455f);
    delta_angle.y = __sinf(j * -0.0015339808305725455f);
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 12, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[0] = *(checksum_DFT + 4096 - 2 + tx + 0);
    shPtr[tx + 0] = rPtr_2[0];
    
    rPtr_2[1] = *(checksum_DFT + 4096 - 2 + tx + 256);
    shPtr[tx + 256] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 4096 - 2 + tx + 512);
    shPtr[tx + 512] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 4096 - 2 + tx + 768);
    shPtr[tx + 768] = rPtr_2[3];
    
    rPtr_2[4] = *(checksum_DFT + 4096 - 2 + tx + 1024);
    shPtr[tx + 1024] = rPtr_2[4];
    
    rPtr_2[5] = *(checksum_DFT + 4096 - 2 + tx + 1280);
    shPtr[tx + 1280] = rPtr_2[5];
    
    rPtr_2[6] = *(checksum_DFT + 4096 - 2 + tx + 1536);
    shPtr[tx + 1536] = rPtr_2[6];
    
    rPtr_2[7] = *(checksum_DFT + 4096 - 2 + tx + 1792);
    shPtr[tx + 1792] = rPtr_2[7];
    
    rPtr_2[8] = *(checksum_DFT + 4096 - 2 + tx + 2048);
    shPtr[tx + 2048] = rPtr_2[8];
    
    rPtr_2[9] = *(checksum_DFT + 4096 - 2 + tx + 2304);
    shPtr[tx + 2304] = rPtr_2[9];
    
    rPtr_2[10] = *(checksum_DFT + 4096 - 2 + tx + 2560);
    shPtr[tx + 2560] = rPtr_2[10];
    
    rPtr_2[11] = *(checksum_DFT + 4096 - 2 + tx + 2816);
    shPtr[tx + 2816] = rPtr_2[11];
    
    rPtr_2[12] = *(checksum_DFT + 4096 - 2 + tx + 3072);
    shPtr[tx + 3072] = rPtr_2[12];
    
    rPtr_2[13] = *(checksum_DFT + 4096 - 2 + tx + 3328);
    shPtr[tx + 3328] = rPtr_2[13];
    
    rPtr_2[14] = *(checksum_DFT + 4096 - 2 + tx + 3584);
    shPtr[tx + 3584] = rPtr_2[14];
    
    rPtr_2[15] = *(checksum_DFT + 4096 - 2 + tx + 3840);
    ...
    gPtr += (bx % BS * 4096);
        ...
        // tmp_3.x += bid_cnt * (rPtr[0].x + rPtr[0].y) * 4096;
    ...
    gPtr += (bx % BS * 4096);
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(2048 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(2048 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(2048 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(2560 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(2560 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(2560 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[13].y + rPtr[13].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[13].y + rPtr[13].x);
        turboFFT_ZMUL(tmp, rPtr[13],r[(2816 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[13], r[(2816 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[13], r[(2816 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[3].y + rPtr[3].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[3].y + rPtr[3].x);
        turboFFT_ZMUL(tmp, rPtr[3],r[(3072 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(3072 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(3072 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[11].y + rPtr[11].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[11].y + rPtr[11].x);
        turboFFT_ZMUL(tmp, rPtr[11],r[(3328 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[11], r[(3328 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[11], r[(3328 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[7].y + rPtr[7].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[7].y + rPtr[7].x);
        turboFFT_ZMUL(tmp, rPtr[7],r[(3584 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(3584 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(3584 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[15].y + rPtr[15].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[15].y + rPtr[15].x);
        turboFFT_ZMUL(tmp, rPtr[15],r[(3840 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[15], r[(3840 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[15], r[(3840 + tx / 1) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 1 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 4096: 0;
        // tmp.y = (tx / 1 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 4096: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 12, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
