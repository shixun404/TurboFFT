...
template<>
__global__ void fft_radix_2<float2, 6, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (64 * BS + 128 - 1) / 128; bid += delta_bid)
    {
    bid_cnt += 1;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 6, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[1] = *(checksum_DFT + 64 - 2 + tx + 16);
    shPtr[tx + 16] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 64 - 2 + tx + 32);
    shPtr[tx + 32] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 64 - 2 + tx + 48);
    ...
    gPtr += (bx % BS * 128);
        ...
        turboFFT_ZMUL(tmp, rPtr[4],r[(8 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[4], r[(8 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[4], r[(8 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[2].y + rPtr[2].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[2].y + rPtr[2].x);
        turboFFT_ZMUL(tmp, rPtr[2],r[(16 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(16 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(16 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[6].y + rPtr[6].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[6].y + rPtr[6].x);
        turboFFT_ZMUL(tmp, rPtr[6],r[(24 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[6], r[(24 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[6], r[(24 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[1].y + rPtr[1].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[1].y + rPtr[1].x);
        turboFFT_ZMUL(tmp, rPtr[1],r[(32 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(32 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(32 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[5].y + rPtr[5].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[5].y + rPtr[5].x);
        turboFFT_ZMUL(tmp, rPtr[5],r[(40 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(40 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(40 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[3].y + rPtr[3].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[3].y + rPtr[3].x);
        turboFFT_ZMUL(tmp, rPtr[3],r[(48 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(48 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(48 + tx / 2) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[7].y + rPtr[7].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[7].y + rPtr[7].x);
        turboFFT_ZMUL(tmp, rPtr[7],r[(56 + tx / 2) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(56 + tx / 2) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(56 + tx / 2) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 2 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 64: 0;
        // tmp.y = (tx / 2 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 64: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 6, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
