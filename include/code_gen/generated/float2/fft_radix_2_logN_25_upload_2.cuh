...
template<>
__global__ void fft_radix_2<float2, 25, 2, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (33554432 * BS + 2048 - 1) / 2048; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    gPtr += (bx % BS * 33554432);
            ...
            *(gPtr + 10485760) = rPtr[10];
            rPtr_4[5].x += rPtr[10].x;
            rPtr_4[5].y += rPtr[10].y;
            
            *(gPtr + 12582912) = rPtr[6];
            rPtr_4[6].x += rPtr[6].x;
            rPtr_4[6].y += rPtr[6].y;
            
            *(gPtr + 14680064) = rPtr[14];
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 25, 2, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 25, 2, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    gPtr += (bx % BS * 33554432);
            ...
            *(gPtr + 0) = rPtr[0];
            ...
            // rPtr_3[1] = *(gPtr + 2097152);
            // turboFFT_ZADD(rPtr_3[1], rPtr_3[1], rPtr[8] );
            // *(gPtr + 2097152) = rPtr_3[1];
            *(gPtr + 2097152) = rPtr[8];
        
            // rPtr_3[2] = *(gPtr + 4194304);
            // turboFFT_ZADD(rPtr_3[2], rPtr_3[2], rPtr[4] );
            // *(gPtr + 4194304) = rPtr_3[2];
            *(gPtr + 4194304) = rPtr[4];
        
            // rPtr_3[3] = *(gPtr + 6291456);
            // turboFFT_ZADD(rPtr_3[3], rPtr_3[3], rPtr[12] );
            // *(gPtr + 6291456) = rPtr_3[3];
            *(gPtr + 6291456) = rPtr[12];
        
            // rPtr_3[4] = *(gPtr + 8388608);
            // turboFFT_ZADD(rPtr_3[4], rPtr_3[4], rPtr[2] );
            // *(gPtr + 8388608) = rPtr_3[4];
            *(gPtr + 8388608) = rPtr[2];
        
            // rPtr_3[5] = *(gPtr + 10485760);
            // turboFFT_ZADD(rPtr_3[5], rPtr_3[5], rPtr[10] );
            // *(gPtr + 10485760) = rPtr_3[5];
            *(gPtr + 10485760) = rPtr[10];
        
            // rPtr_3[6] = *(gPtr + 12582912);
            // turboFFT_ZADD(rPtr_3[6], rPtr_3[6], rPtr[6] );
            // *(gPtr + 12582912) = rPtr_3[6];
            *(gPtr + 12582912) = rPtr[6];
        
            // rPtr_3[7] = *(gPtr + 14680064);
            // turboFFT_ZADD(rPtr_3[7], rPtr_3[7], rPtr[14] );
            // *(gPtr + 14680064) = rPtr_3[7];
            *(gPtr + 14680064) = rPtr[14];
        
            // rPtr_3[8] = *(gPtr + 16777216);
            // turboFFT_ZADD(rPtr_3[8], rPtr_3[8], rPtr[1] );
            // *(gPtr + 16777216) = rPtr_3[8];
            *(gPtr + 16777216) = rPtr[1];
        
            // rPtr_3[9] = *(gPtr + 18874368);
            // turboFFT_ZADD(rPtr_3[9], rPtr_3[9], rPtr[9] );
            // *(gPtr + 18874368) = rPtr_3[9];
            *(gPtr + 18874368) = rPtr[9];
        
            // rPtr_3[10] = *(gPtr + 20971520);
            // turboFFT_ZADD(rPtr_3[10], rPtr_3[10], rPtr[5] );
            // *(gPtr + 20971520) = rPtr_3[10];
            *(gPtr + 20971520) = rPtr[5];
        
            // rPtr_3[11] = *(gPtr + 23068672);
            // turboFFT_ZADD(rPtr_3[11], rPtr_3[11], rPtr[13] );
            // *(gPtr + 23068672) = rPtr_3[11];
            *(gPtr + 23068672) = rPtr[13];
        
            // rPtr_3[12] = *(gPtr + 25165824);
            // turboFFT_ZADD(rPtr_3[12], rPtr_3[12], rPtr[3] );
            // *(gPtr + 25165824) = rPtr_3[12];
            *(gPtr + 25165824) = rPtr[3];
        
            // rPtr_3[13] = *(gPtr + 27262976);
            // turboFFT_ZADD(rPtr_3[13], rPtr_3[13], rPtr[11] );
            // *(gPtr + 27262976) = rPtr_3[13];
            *(gPtr + 27262976) = rPtr[11];
        
            // rPtr_3[14] = *(gPtr + 29360128);
            // turboFFT_ZADD(rPtr_3[14], rPtr_3[14], rPtr[7] );
            // *(gPtr + 29360128) = rPtr_3[14];
            *(gPtr + 29360128) = rPtr[7];
        
            // rPtr_3[15] = *(gPtr + 31457280);
            // turboFFT_ZADD(rPtr_3[15], rPtr_3[15], rPtr[15] );
            // *(gPtr + 31457280) = rPtr_3[15];
            *(gPtr + 31457280) = rPtr[15];
        }}