...
template<>
__global__ void fft_radix_2<float2, 30, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (1073741824 * BS + 8192 - 1) / 8192; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    gPtr += (bx % 1) * 1024 * 1048576;
    bx = bx / 1;
    
    gPtr += (bx % BS * 1073741824);
    
        rPtr[0] = *(gPtr + 0);
        rPtr_3[0].x += rPtr[0].x;
        rPtr_3[0].y += rPtr[0].y;
        
        rPtr[1] = *(gPtr + 67108864);
        rPtr_3[1].x += rPtr[1].x;
        rPtr_3[1].y += rPtr[1].y;
        
        rPtr[2] = *(gPtr + 134217728);
        rPtr_3[2].x += rPtr[2].x;
        rPtr_3[2].y += rPtr[2].y;
        
        rPtr[3] = *(gPtr + 201326592);
        rPtr_3[3].x += rPtr[3].x;
        rPtr_3[3].y += rPtr[3].y;
        
        rPtr[4] = *(gPtr + 268435456);
        rPtr_3[4].x += rPtr[4].x;
        rPtr_3[4].y += rPtr[4].y;
        
        rPtr[5] = *(gPtr + 335544320);
        rPtr_3[5].x += rPtr[5].x;
        rPtr_3[5].y += rPtr[5].y;
        
        rPtr[6] = *(gPtr + 402653184);
        rPtr_3[6].x += rPtr[6].x;
        rPtr_3[6].y += rPtr[6].y;
        
        rPtr[7] = *(gPtr + 469762048);
        rPtr_3[7].x += rPtr[7].x;
        rPtr_3[7].y += rPtr[7].y;
        
        rPtr[8] = *(gPtr + 536870912);
        rPtr_3[8].x += rPtr[8].x;
        rPtr_3[8].y += rPtr[8].y;
        
        rPtr[9] = *(gPtr + 603979776);
        rPtr_3[9].x += rPtr[9].x;
        rPtr_3[9].y += rPtr[9].y;
        
        rPtr[10] = *(gPtr + 671088640);
        rPtr_3[10].x += rPtr[10].x;
        rPtr_3[10].y += rPtr[10].y;
        
        rPtr[11] = *(gPtr + 738197504);
        rPtr_3[11].x += rPtr[11].x;
        rPtr_3[11].y += rPtr[11].y;
        
        rPtr[12] = *(gPtr + 805306368);
        rPtr_3[12].x += rPtr[12].x;
        rPtr_3[12].y += rPtr[12].y;
        
        rPtr[13] = *(gPtr + 872415232);
        rPtr_3[13].x += rPtr[13].x;
        rPtr_3[13].y += rPtr[13].y;
        
        rPtr[14] = *(gPtr + 939524096);
        rPtr_3[14].x += rPtr[14].x;
        rPtr_3[14].y += rPtr[14].y;
        
        rPtr[15] = *(gPtr + 1006632960);
    ...
    global_k += tx / 8;
        ...
        angle.x = __cosf( global_j * global_k * -5.8516723170686385e-09f);
        angle.y = __sinf( global_j * global_k * -5.8516723170686385e-09f);
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 30, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<float2, 30, 0, 1, 1>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    global_k += tx / 8;
        ...
        turboFFT_ZMUL(angle, tmp, delta_angle);
            ...
            *(gPtr + 0) = rPtr[0];
            ...
            // rPtr_3[1] = *(gPtr + 67108864);
            // turboFFT_ZADD(rPtr_3[1], rPtr_3[1], rPtr[1] );
            // *(gPtr + 67108864) = rPtr_3[1];
            *(gPtr + 67108864) = rPtr[1];
        
            // rPtr_3[2] = *(gPtr + 134217728);
            // turboFFT_ZADD(rPtr_3[2], rPtr_3[2], rPtr[2] );
            // *(gPtr + 134217728) = rPtr_3[2];
            *(gPtr + 134217728) = rPtr[2];
        
            // rPtr_3[3] = *(gPtr + 201326592);
            // turboFFT_ZADD(rPtr_3[3], rPtr_3[3], rPtr[3] );
            // *(gPtr + 201326592) = rPtr_3[3];
            *(gPtr + 201326592) = rPtr[3];
        
            // rPtr_3[4] = *(gPtr + 268435456);
            // turboFFT_ZADD(rPtr_3[4], rPtr_3[4], rPtr[8] );
            // *(gPtr + 268435456) = rPtr_3[4];
            *(gPtr + 268435456) = rPtr[8];
        
            // rPtr_3[5] = *(gPtr + 335544320);
            // turboFFT_ZADD(rPtr_3[5], rPtr_3[5], rPtr[9] );
            // *(gPtr + 335544320) = rPtr_3[5];
            *(gPtr + 335544320) = rPtr[9];
        
            // rPtr_3[6] = *(gPtr + 402653184);
            // turboFFT_ZADD(rPtr_3[6], rPtr_3[6], rPtr[10] );
            // *(gPtr + 402653184) = rPtr_3[6];
            *(gPtr + 402653184) = rPtr[10];
        
            // rPtr_3[7] = *(gPtr + 469762048);
            // turboFFT_ZADD(rPtr_3[7], rPtr_3[7], rPtr[11] );
            // *(gPtr + 469762048) = rPtr_3[7];
            *(gPtr + 469762048) = rPtr[11];
        
            // rPtr_3[8] = *(gPtr + 536870912);
            // turboFFT_ZADD(rPtr_3[8], rPtr_3[8], rPtr[4] );
            // *(gPtr + 536870912) = rPtr_3[8];
            *(gPtr + 536870912) = rPtr[4];
        
            // rPtr_3[9] = *(gPtr + 603979776);
            // turboFFT_ZADD(rPtr_3[9], rPtr_3[9], rPtr[5] );
            // *(gPtr + 603979776) = rPtr_3[9];
            *(gPtr + 603979776) = rPtr[5];
        
            // rPtr_3[10] = *(gPtr + 671088640);
            // turboFFT_ZADD(rPtr_3[10], rPtr_3[10], rPtr[6] );
            // *(gPtr + 671088640) = rPtr_3[10];
            *(gPtr + 671088640) = rPtr[6];
        
            // rPtr_3[11] = *(gPtr + 738197504);
            // turboFFT_ZADD(rPtr_3[11], rPtr_3[11], rPtr[7] );
            // *(gPtr + 738197504) = rPtr_3[11];
            *(gPtr + 738197504) = rPtr[7];
        
            // rPtr_3[12] = *(gPtr + 805306368);
            // turboFFT_ZADD(rPtr_3[12], rPtr_3[12], rPtr[12] );
            // *(gPtr + 805306368) = rPtr_3[12];
            *(gPtr + 805306368) = rPtr[12];
        
            // rPtr_3[13] = *(gPtr + 872415232);
            // turboFFT_ZADD(rPtr_3[13], rPtr_3[13], rPtr[13] );
            // *(gPtr + 872415232) = rPtr_3[13];
            *(gPtr + 872415232) = rPtr[13];
        
            // rPtr_3[14] = *(gPtr + 939524096);
            // turboFFT_ZADD(rPtr_3[14], rPtr_3[14], rPtr[14] );
            // *(gPtr + 939524096) = rPtr_3[14];
            *(gPtr + 939524096) = rPtr[14];
        
            // rPtr_3[15] = *(gPtr + 1006632960);
            // turboFFT_ZADD(rPtr_3[15], rPtr_3[15], rPtr[15] );
            // *(gPtr + 1006632960) = rPtr_3[15];
            *(gPtr + 1006632960) = rPtr[15];
        }}