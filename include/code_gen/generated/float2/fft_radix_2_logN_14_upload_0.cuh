...
__global__ void fft_radix_2<float2, 14, 0, 0, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (16384 * BS + 512 - 1) / 512; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    global_k += tx / 4;
        ...
        delta_angle.x = __cosf(global_j *  -0.006135923322290182f);
        delta_angle.y = __sinf(global_j *  -0.006135923322290182f);
        angle.x = __cosf( global_j * global_k * -0.0003834951969714103f);
        angle.y = __sinf( global_j * global_k * -0.0003834951969714103f);
...
__global__ void fft_radix_2<float2, 14, 0, 1, 0>(float2* inputs, float2* outputs, float2* twiddle, float2* checksum_DFT, int BS, int thread_bs) {
    ...
    rPtr_2[1] = *(checksum_DFT + 128 - 2 + tx + 64);
    ...
    global_k += tx / 4;
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(16 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(16 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(16 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[2],r[(32 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(32 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(32 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[3],r[(48 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(48 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(48 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[4],r[(64 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[4], r[(64 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[4], r[(64 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(80 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(80 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(80 + tx / 4) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(112 + tx / 4) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(112 + tx / 4) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(112 + tx / 4) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 4 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 128: 0;
        // tmp.y = (tx / 4 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 128: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
