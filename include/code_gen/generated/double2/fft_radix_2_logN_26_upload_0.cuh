...
template<>
__global__ void fft_radix_2<double2, 26, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (67108864 * BS + 4096 - 1) / 4096; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    global_k += tx / 16;
        ...
        delta_angle = twiddle[67108863 + global_j * (16)];
        angle = twiddle[67108863 + global_j * global_k];
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 26, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 26, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
