...
template<>
__global__ void fft_radix_2<double2, 7, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 7, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[1] = *(checksum_DFT + 128 - 2 + tx + 16);
    shPtr[tx + 16] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 128 - 2 + tx + 32);
    shPtr[tx + 32] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 128 - 2 + tx + 48);
    shPtr[tx + 48] = rPtr_2[3];
    
    rPtr_2[4] = *(checksum_DFT + 128 - 2 + tx + 64);
    shPtr[tx + 64] = rPtr_2[4];
    
    rPtr_2[5] = *(checksum_DFT + 128 - 2 + tx + 80);
    shPtr[tx + 80] = rPtr_2[5];
    
    rPtr_2[6] = *(checksum_DFT + 128 - 2 + tx + 96);
    shPtr[tx + 96] = rPtr_2[6];
    
    rPtr_2[7] = *(checksum_DFT + 128 - 2 + tx + 112);
    ...
    gPtr += (bx % BS * 128);
        ...
        turboFFT_ZMUL(tmp, rPtr[1],r[(16 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(16 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(16 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[2].y + rPtr[2].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[2].y + rPtr[2].x);
        turboFFT_ZMUL(tmp, rPtr[2],r[(32 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(32 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(32 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[4],r[(64 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[4], r[(64 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[4], r[(64 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[5].y + rPtr[5].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[5].y + rPtr[5].x);
        turboFFT_ZMUL(tmp, rPtr[5],r[(80 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(80 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(80 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(112 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(112 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(112 + tx / 1) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 1 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 128: 0;
        // tmp.y = (tx / 1 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 128: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 7, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
