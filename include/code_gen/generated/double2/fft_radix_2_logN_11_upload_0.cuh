...
template<>
__global__ void fft_radix_2<double2, 11, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    shPtr[offset + 1 * (1 + threadIdx.x % 4) % 4 + (1 / 4) * 4] = rPtr[4];
    ...
    shPtr[offset + 1 * (2 + threadIdx.x % 4) % 4 + (2 / 4) * 4] = rPtr[2];
    ...
    shPtr[offset + 1 * (3 + threadIdx.x % 4) % 4 + (3 / 4) * 4] = rPtr[6];
    ...
    shPtr[offset + 1 * (4 + threadIdx.x % 4) % 4 + (4 / 4) * 4] = rPtr[1];
    ...
    shPtr[offset + 1 * (5 + threadIdx.x % 4) % 4 + (5 / 4) * 4] = rPtr[5];
    ...
    shPtr[offset + 1 * (6 + threadIdx.x % 4) % 4 + (6 / 4) * 4] = rPtr[3];
    ...
    shPtr[offset + 1 * (7 + threadIdx.x % 4) % 4 + (7 / 4) * 4] = rPtr[7];
    
    offset = 0;
    offset += (tx / 8) * 8 + 
              ((tx % 8) / 4) * 4 + (tx % 4 + tx / 8) % 4;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 11, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    gPtr += (bx % BS * 2048);
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(768 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(768 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(768 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[3],r[(1280 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(1280 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(1280 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(1792 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(1792 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(1792 + tx / 1) % 3])
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 11, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
