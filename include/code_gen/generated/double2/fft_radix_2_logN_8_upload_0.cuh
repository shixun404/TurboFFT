...
template<>
__global__ void fft_radix_2<double2, 8, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 8, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[1] = *(checksum_DFT + 256 - 2 + tx + 16);
    shPtr[tx + 16] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 256 - 2 + tx + 32);
    shPtr[tx + 32] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 256 - 2 + tx + 48);
    shPtr[tx + 48] = rPtr_2[3];
    
    rPtr_2[4] = *(checksum_DFT + 256 - 2 + tx + 64);
    shPtr[tx + 64] = rPtr_2[4];
    
    rPtr_2[5] = *(checksum_DFT + 256 - 2 + tx + 80);
    shPtr[tx + 80] = rPtr_2[5];
    
    rPtr_2[6] = *(checksum_DFT + 256 - 2 + tx + 96);
    shPtr[tx + 96] = rPtr_2[6];
    
    rPtr_2[7] = *(checksum_DFT + 256 - 2 + tx + 112);
    shPtr[tx + 112] = rPtr_2[7];
    
    rPtr_2[8] = *(checksum_DFT + 256 - 2 + tx + 128);
    shPtr[tx + 128] = rPtr_2[8];
    
    rPtr_2[9] = *(checksum_DFT + 256 - 2 + tx + 144);
    shPtr[tx + 144] = rPtr_2[9];
    
    rPtr_2[10] = *(checksum_DFT + 256 - 2 + tx + 160);
    shPtr[tx + 160] = rPtr_2[10];
    
    rPtr_2[11] = *(checksum_DFT + 256 - 2 + tx + 176);
    shPtr[tx + 176] = rPtr_2[11];
    
    rPtr_2[12] = *(checksum_DFT + 256 - 2 + tx + 192);
    shPtr[tx + 192] = rPtr_2[12];
    
    rPtr_2[13] = *(checksum_DFT + 256 - 2 + tx + 208);
    shPtr[tx + 208] = rPtr_2[13];
    
    rPtr_2[14] = *(checksum_DFT + 256 - 2 + tx + 224);
    shPtr[tx + 224] = rPtr_2[14];
    
    rPtr_2[15] = *(checksum_DFT + 256 - 2 + tx + 240);
    shPtr[tx + 240] = rPtr_2[15];
    ...
    gPtr += (bx % BS * 256);
        ...
        turboFFT_ZMUL(tmp, rPtr[8],r[(16 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[8], r[(16 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[8], r[(16 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[4].y + rPtr[4].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[4].y + rPtr[4].x);
        turboFFT_ZMUL(tmp, rPtr[4],r[(32 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[4], r[(32 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[4], r[(32 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[12].y + rPtr[12].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[12].y + rPtr[12].x);
        turboFFT_ZMUL(tmp, rPtr[12],r[(48 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[12], r[(48 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[12], r[(48 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[2].y + rPtr[2].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[2].y + rPtr[2].x);
        turboFFT_ZMUL(tmp, rPtr[2],r[(64 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(64 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(64 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[10].y + rPtr[10].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[10].y + rPtr[10].x);
        turboFFT_ZMUL(tmp, rPtr[10],r[(80 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[10], r[(80 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[10], r[(80 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[6].y + rPtr[6].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[6].y + rPtr[6].x);
        turboFFT_ZMUL(tmp, rPtr[6],r[(96 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[6], r[(96 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[6], r[(96 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[14].y + rPtr[14].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[14].y + rPtr[14].x);
        turboFFT_ZMUL(tmp, rPtr[14],r[(112 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[14], r[(112 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[14], r[(112 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[9],r[(144 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[9], r[(144 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[9], r[(144 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[5].y + rPtr[5].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[5].y + rPtr[5].x);
        turboFFT_ZMUL(tmp, rPtr[5],r[(160 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(160 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(160 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[13].y + rPtr[13].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[13].y + rPtr[13].x);
        turboFFT_ZMUL(tmp, rPtr[13],r[(176 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[13], r[(176 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[13], r[(176 + tx / 1) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[11],r[(208 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[11], r[(208 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[11], r[(208 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[7].y + rPtr[7].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[7].y + rPtr[7].x);
        turboFFT_ZMUL(tmp, rPtr[7],r[(224 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(224 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(224 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[15].y + rPtr[15].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[15].y + rPtr[15].x);
        turboFFT_ZMUL(tmp, rPtr[15],r[(240 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[15], r[(240 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[15], r[(240 + tx / 1) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 1 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 256: 0;
        // tmp.y = (tx / 1 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 256: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 8, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
