...
template<>
__global__ void fft_radix_2<double2, 5, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (32 * BS + 256 - 1) / 256; bid += delta_bid)
    {
    bid_cnt += 1;
    ...
    rPtr[4] = shPtr[offset + 128];
    
    rPtr[5] = shPtr[offset + 160];
    
    rPtr[6] = shPtr[offset + 192];
    
    rPtr[7] = shPtr[offset + 224];
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 5, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    gPtr += (bx % BS * 256);
        ...
        turboFFT_ZMUL(tmp, rPtr[5],r[(12 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(12 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(12 + tx / 8) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[3],r[(20 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(20 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(20 + tx / 8) % 3])
        ...
        turboFFT_ZMUL(tmp, rPtr[7],r[(28 + tx / 8) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(28 + tx / 8) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(28 + tx / 8) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 8 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 32: 0;
        // tmp.y = (tx / 8 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 32: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 5, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
