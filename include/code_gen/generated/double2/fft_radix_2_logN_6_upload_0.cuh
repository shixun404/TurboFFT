...
template<>
__global__ void fft_radix_2<double2, 6, 0, 0, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    for(bid = (blockIdx.x / tb_gap) * tb_gap * thread_bs + blockIdx.x % tb_gap;
                bid_cnt < thread_bs && bid < (64 * BS + 64 - 1) / 64; bid += delta_bid)
    {
    bid_cnt += 1;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 6, 0, 1, 0>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    rPtr_2[0] = *(checksum_DFT + 64 - 2 + tx + 0);
    shPtr[tx + 0] = rPtr_2[0];
    
    rPtr_2[1] = *(checksum_DFT + 64 - 2 + tx + 8);
    shPtr[tx + 8] = rPtr_2[1];
    
    rPtr_2[2] = *(checksum_DFT + 64 - 2 + tx + 16);
    shPtr[tx + 16] = rPtr_2[2];
    
    rPtr_2[3] = *(checksum_DFT + 64 - 2 + tx + 24);
    shPtr[tx + 24] = rPtr_2[3];
    
    rPtr_2[4] = *(checksum_DFT + 64 - 2 + tx + 32);
    shPtr[tx + 32] = rPtr_2[4];
    
    rPtr_2[5] = *(checksum_DFT + 64 - 2 + tx + 40);
    shPtr[tx + 40] = rPtr_2[5];
    
    rPtr_2[6] = *(checksum_DFT + 64 - 2 + tx + 48);
    shPtr[tx + 48] = rPtr_2[6];
    
    rPtr_2[7] = *(checksum_DFT + 64 - 2 + tx + 56);
    shPtr[tx + 56] = rPtr_2[7];
    ...
    gPtr += (bx % BS * 64);
        ...
        // tmp_3.x += bid_cnt * (rPtr[0].x + rPtr[0].y) * 64;
    ...
    gPtr += (bx % BS * 64);
        ...
        turboFFT_ZMUL(tmp, rPtr[4],r[(8 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[4], r[(8 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[4], r[(8 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[2].y + rPtr[2].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[2].y + rPtr[2].x);
        turboFFT_ZMUL(tmp, rPtr[2],r[(16 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[2], r[(16 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[2], r[(16 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[6].y + rPtr[6].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[6].y + rPtr[6].x);
        turboFFT_ZMUL(tmp, rPtr[6],r[(24 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[6], r[(24 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[6], r[(24 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[1].y + rPtr[1].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[1].y + rPtr[1].x);
        turboFFT_ZMUL(tmp, rPtr[1],r[(32 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[1], r[(32 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[1], r[(32 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[5].y + rPtr[5].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[5].y + rPtr[5].x);
        turboFFT_ZMUL(tmp, rPtr[5],r[(40 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[5], r[(40 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[5], r[(40 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[3].y + rPtr[3].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[3].y + rPtr[3].x);
        turboFFT_ZMUL(tmp, rPtr[3],r[(48 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[3], r[(48 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[3], r[(48 + tx / 1) % 3])
        
        // 1's vector
        // tmp_3.y -=  (rPtr[7].y + rPtr[7].x) * bid_cnt;
        // tmp_1.y -=  (rPtr[7].y + rPtr[7].x);
        turboFFT_ZMUL(tmp, rPtr[7],r[(56 + tx / 1) % 3])
        tmp_1.y -= (tmp.x + tmp.y);
        tmp_3.y -= (tmp.y + tmp.x) * bid_cnt;
        // turboFFT_ZMUL_NACC(tmp_1,  rPtr[7], r[(56 + tx / 1) % 3])
        // turboFFT_ZMUL_NACC(tmp_3,  rPtr[7], r[(56 + tx / 1) % 3])
        ...
        // 1's vector
        // tmp.x = (tx / 1 == 0) ? (rPtr_3[0].y + rPtr_3[0].x) * 64: 0;
        // tmp.y = (tx / 1 == 0) ? (abs(rPtr_3[0].y) + abs(rPtr_3[0].x)) * 64: 0;
        tmp = tmp_1;
        tmp_1.y += tmp.x;
...
#include "../../../TurboFFT_radix_2_template.h"
template<>
__global__ void fft_radix_2<double2, 6, 0, 1, 1>(double2* inputs, double2* outputs, double2* twiddle, double2* checksum_DFT, int BS, int thread_bs) {
    int bid_cnt = 0;
    ...
    gPtr += (bx % BS * 64);
            ...
            // turboFFT_ZSUB(rPtr[4], rPtr[4], rPtr_4[1]);
            
            // turboFFT_ZSUB(rPtr[2], rPtr[2], rPtr_4[2]);
            
            // turboFFT_ZSUB(rPtr[6], rPtr[6], rPtr_4[3]);
            
            // turboFFT_ZSUB(rPtr[1], rPtr[1], rPtr_4[4]);
            
            // turboFFT_ZSUB(rPtr[5], rPtr[5], rPtr_4[5]);
            
            // turboFFT_ZSUB(rPtr[3], rPtr[3], rPtr_4[6]);
            ...
            *(gPtr + 0) = rPtr[0];
            ...
            // rPtr_3[1] = *(gPtr + 8);
            // turboFFT_ZADD(rPtr_3[1], rPtr_3[1], rPtr[4] );
            // *(gPtr + 8) = rPtr_3[1];
            *(gPtr + 8) = rPtr[4];
            ...
            *(gPtr + 16) = rPtr[2];
            ...
            // rPtr_3[3] = *(gPtr + 24);
            // turboFFT_ZADD(rPtr_3[3], rPtr_3[3], rPtr[6] );
            // *(gPtr + 24) = rPtr_3[3];
            *(gPtr + 24) = rPtr[6];
        
            // rPtr_3[4] = *(gPtr + 32);
            // turboFFT_ZADD(rPtr_3[4], rPtr_3[4], rPtr[1] );
            // *(gPtr + 32) = rPtr_3[4];
            *(gPtr + 32) = rPtr[1];
            ...
            *(gPtr + 40) = rPtr[5];
            ...
            // rPtr_3[6] = *(gPtr + 48);
            // turboFFT_ZADD(rPtr_3[6], rPtr_3[6], rPtr[3] );
            // *(gPtr + 48) = rPtr_3[6];
            *(gPtr + 48) = rPtr[3];
