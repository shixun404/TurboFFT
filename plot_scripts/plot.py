...
filename = f'../artifact_data/Benchmark=1_TurboFFT_FP32.csv'
try:
    f = open(filename, 'r')
except:
...
filename = '../artifact_data/Benchmark=1_TurboFFT_FP64.csv'
f = open(filename, 'r')
lines = f.readlines()
for line in lines:
    if line.split(',')[0] != 'turboFFT':
...
for p in range(2):
    precision = 32 + 32 * p
    filename_list = [f'../artifact_data/Benchmark=1_cuFFT_FP{precision}_FT_OFFLINE.csv', f'../artifact_data/Benchmark=1_TurboFFT_FP{precision}_FT_Thread.csv'] + [f'../artifact_data/Benchmark=1_TurboFFT_FP{precision}_FT_BS={2 ** i}.csv' for 
                    i in range(3)]
