...
class TurboFFT:
    ...
    def save_generated_code(self, ):
        if not self.if_special:
            ...
            for i in range(1):
                file_name = f"../generated/{self.data_type}/fft_radix_{self.radix}_logN_{int(log(N, 2))}_upload_{i}_stride_DY.cuh"
            ...
            for i in range(1):
                if i != 0:
                    ...
                    file_name = f"../generated/{self.data_type}/fft_radix_{self.radix}_logN_{int(log(N, 2))}_upload_{0}_stride_DY.cuh"
                    # if self.ft == 0:
                    if ( self.ft == 0 and self.if_thread_ft == 0) or self.if_write is True:
                        with open(file_name, 'w') as f:
...
__global__ void fft_{int(log(N, self.radix))}_stride_DY''' \
        + f'''(float2* gPtr_1, float2* outputs, int threadblock_bs, int global_bs)''' + ''' {
    int bid_cnt = 0;
    '''
    #     head += f'''
...
'''
            ...
            globalAccess_code += f'''{self.gPtr} = outputs + bid_itr * gridDim.x * threadblock_bs * 64;
            {self.gPtr} += threadIdx.x % {global_tensor_shape[dim] // WorkerFFTSize};
    {self.gPtr} += (blockIdx.x * threadblock_bs + threadIdx.x / {global_tensor_shape[dim] // WorkerFFTSize}) * 64;
    '''
