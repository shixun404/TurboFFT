...
class TurboFFT:
    ...
    def save_generated_code(self, ):
        if not self.if_special:
            ...
            for i in range(1):
                file_name = f"../generated/{self.data_type}/ifft_radix_{self.radix}_logN_{int(log(N, 2))}_upload_{i}_stride_DY.cuh"
            ...
            for i in range(1):
                if i != 0:
                    ...
                    file_name = f"../generated/{self.data_type}/ifft_radix_{self.radix}_logN_{int(log(N, 2))}_upload_{0}_stride_DY.cuh"
                    # if self.ft == 0:
                    if ( self.ft == 0 and self.if_thread_ft == 0) or self.if_write is True:
                        with open(file_name, 'w') as f:
...
__global__ void ifft_{int(log(N, self.radix))}_stride_DY''' \
