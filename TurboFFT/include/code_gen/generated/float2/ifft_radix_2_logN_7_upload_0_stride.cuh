...
__global__ void ifft_7_stride(float2* gPtr_1, float2* outputs, int threadblock_bs, int DY, int global_bs, int dimX) {
    int bid_cnt = 0;
    int j;
    int k;
    int global_j;
...
gPtr = gPtr_1 + bid_itr * ((gridDim.x % (DY / threadblock_bs)) * threadblock_bs + (gridDim.x / (DY / threadblock_bs)) * DY * dimX);
        
    bx = blockIdx.x;
    tx = threadIdx.x;
    
            gPtr += threadIdx.x % 8 * DY;
    
    gPtr += ((blockIdx.x % (DY / threadblock_bs)) * threadblock_bs + threadIdx.x / 8) + (blockIdx.x / (DY / threadblock_bs)) * DY * dimX;
    ...
    offset += ((threadIdx.x / 1) % 8) * 16;
    
    offset += (threadIdx.x / 8) * 128;
    ...
    rPtr[8] = shPtr[offset + 64];
    
    rPtr[9] = shPtr[offset + 72];
    
    rPtr[10] = shPtr[offset + 80];
    
    rPtr[11] = shPtr[offset + 88];
    
    rPtr[12] = shPtr[offset + 96];
    
    rPtr[13] = shPtr[offset + 104];
    
    rPtr[14] = shPtr[offset + 112];
    
    rPtr[15] = shPtr[offset + 120];
    ...
    tx = threadIdx.x;
        ...
        gPtr = outputs + bid_itr * ((gridDim.x % (DY / threadblock_bs)) * threadblock_bs + (gridDim.x / (DY / threadblock_bs)) * DY * 128);
        gPtr += threadIdx.x % 8 * DY;
    
    gPtr += ((blockIdx.x % (DY / threadblock_bs)) * threadblock_bs + threadIdx.x / 8) + (blockIdx.x / (DY / threadblock_bs)) * DY * 128;
