...
__global__ void ifft_9_stride(float2* gPtr_1, float2* outputs, int threadblock_bs, int DY, int global_bs, int dimX) {
    int bid_cnt = 0;
    int j;
    int k;
    int global_j;
...
gPtr = gPtr_1 + bid_itr * ((gridDim.x % (DY / threadblock_bs)) * threadblock_bs + (gridDim.x / (DY / threadblock_bs)) * DY * dimX);
    ...
    gPtr += ((blockIdx.x % (DY / threadblock_bs)) * threadblock_bs + threadIdx.x / 64) + (blockIdx.x / (DY / threadblock_bs)) * DY * dimX;
    ...
    tx = threadIdx.x;
        ...
        gPtr = outputs + bid_itr * ((gridDim.x % (DY / threadblock_bs)) * threadblock_bs + (gridDim.x / (DY / threadblock_bs)) * DY * 512);
        gPtr += threadIdx.x % 64 * DY;
