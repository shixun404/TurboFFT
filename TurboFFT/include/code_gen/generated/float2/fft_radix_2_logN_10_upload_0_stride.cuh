...
__global__ void fft_10_stride(float2* gPtr_1, float2* outputs, int threadblock_bs, int DY, int global_bs, int dimX) {
    int bid_cnt = 0;
    int j;
    int k;
    int global_j;
...
gPtr = gPtr_1 + bid_itr * ((gridDim.x % (DY / threadblock_bs)) * threadblock_bs + (gridDim.x / (DY / threadblock_bs)) * DY * 1024);
