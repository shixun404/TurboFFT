...
__global__ void ifft_7_stride_DY(float2* gPtr_1, float2* outputs, int threadblock_bs, int global_bs) {
    int bid_cnt = 0;
    int j;
    int k;
    int global_j;
...
gPtr = gPtr_1 + bid_itr * gridDim.x * threadblock_bs * 64;
    ...
    gPtr += (blockIdx.x * threadblock_bs + threadIdx.x / 8) * 64;
    ...
    tx = threadIdx.x;
        ...
        gPtr = outputs + bid_itr * gridDim.x * threadblock_bs * 128;
        gPtr += threadIdx.x % 8;
    
    gPtr += (blockIdx.x * threadblock_bs + threadIdx.x / 8) * 128;
