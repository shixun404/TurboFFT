...
__global__ void fft_7_stride_DY(float2* gPtr_1, float2* outputs, int threadblock_bs, int global_bs) {
    int bid_cnt = 0;
    int j;
    int k;
    int global_j;
...
gPtr = gPtr_1 + bid_itr * gridDim.x * threadblock_bs * 128;
    ...
    bx = blockIdx.x;
    tx = threadIdx.x;
    gPtr = outputs + bid_itr * gridDim.x * threadblock_bs * 64;
            gPtr += threadIdx.x % 8;
